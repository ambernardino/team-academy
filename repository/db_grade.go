...
func CreateGradeTableIfNotExists(db *gorm.DB) (err error) {
...
func GiveGrade(db *gorm.DB, grade Grade) (err error) {
	_, err = db.Table("student").Select("student.id, student_subject.subject_id").Joins("JOIN student_subject ON student_subject.student_id = student.id").Where(&StudentSubject{SubjectID: grade.SubjectID}).Rows()
